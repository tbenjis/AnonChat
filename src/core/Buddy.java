...
public class Buddy {
	...
	String address;
	private String name; // wtf is this for?, custom name? todo use this
							// somewhere -- used
	public volatile Socket ourSock = null; // Our socket to them - the output
											// sock
	public volatile OutputStreamWriter ourSockOut = null;
	public volatile Socket theirSock = null; // Their socket to us - the input
												// sock
	String cookie = null;
		...
		if (!getBlack(this.address)) {
			if (ourSock != null) {
				...
				Logger.log(Logger.WARNING, this,
						"Connect(V)V was called but ourSock isn't null!");
				Thread.dumpStack();
				return;
			}
				...
				public void run() {
					if (!getBlack(Buddy.this.address)) {
						try {
							...
							ourSock = new Socket(new Proxy(Proxy.Type.SOCKS,
									new InetSocketAddress("127.0.0.1",
											Config.SOCKS_PORT)));
							ourSock.connect(InetSocketAddress.createUnresolved(
									address + ".onion", 11009));
							setStatus(HANDSHAKE);
							Logger.log(Logger.INFO, Buddy.this,
									"Connected to {" + address + ", " + name
											+ "}");
							// ourSockOut = ourSock.getOutputStream();
							ourSockOut = new OutputStreamWriter(ourSock
									.getOutputStream(), "UTF-8");
							sendPing();
							if (theirCookie != null) {
								sendPong(theirCookie);
								Logger.log(Logger.DEBUG, Buddy.this, "Sent "
										+ address + " a cached pong");
								theirCookie = null; // cbb to clear properly
													// elsewhere
							}
							connectTime = -1;
							if (ourSock == null)
						...
						synchronized (connectLock) {
							connectLock.notifyAll(); // incase something messed
														// up we clear it out of
														// the way so it should
														// work next time
						}
						if (ourSockOut != null) {
							try {
								...
								while ((b = (byte) is.read()) != -1) {
									...
									if ((char) b == ' '
											&& !s.substring(0, s.length() - 1)
													.contains(" ")) {
										BuddyIncoming.init_outin(s, Buddy.this,
												is);
										s = "";
									}
								}
	...
	protected void setStatus(byte nstatus) {
		if (nstatus == OFFLINE) {
			...
			// Thread.dumpStack();
			reconnectAt = connectFailCount < 4 ? (15)
					: connectFailCount < 15 ? (5 * 60) : (30 * 60);
		...
		if (nstatus == HANDSHAKE && status == OFFLINE) {
			lastPing = System.currentTimeMillis(); // During handshake
													// lastStatus is used as
													// lastPing
		}
		if (nstatus >= ONLINE && status <= HANDSHAKE) { // if connection just
														// finished
			...
			Logger.log(
					...
					address
							+ " connected for "
							+ (connectedAt == -1 ? " Not Set" : ((System
									.currentTimeMillis() - connectedAt) / 1000)));
			connectedAt = -1;
		}
	...
	public void sendRaw(String command) throws IOException {
		if (!getBlack(this.address)) {
			synchronized (OSO_LOCK) {
				try {
					Logger.log(Logger.DEBUG, this, "Send " + address + " "
							+ command);
	...
	public void onFullyConnected() throws IOException {
		...
		sendProfileName();
		sendProfileText();
		if (true) // check if already added? - one of those things that's always
					// sent?
			sendAddMe();
	...
	public void attatch(Socket s, Scanner sc) throws IOException {
		...
		if (!getBlack(this.address)) {
			if (theirSock != null) {
				disconnect();
				connect(); // TODO might need to do something about this entire
							// block
				synchronized (connectLock) {
					try {
						...
						// !NOTE! notify is called regardless of success or
						// failure
			...
			// FIXME really need to fix replying to commands before we're
			// connected
			this.theirSock = s;
			...
			try {
				...
				Logger.log(Logger.DEBUG, this, "[" + address + "] attatch() "
						+ se.getLocalizedMessage() + " | "
						+ se.getStackTrace()[0]);
				// SocketExceptions are quite common and generally nothing to
				// worry about
			} catch (IOException e) {
				Logger.log(
						Logger.WARNING,
						this,
						"[" + address + "] theirSock = null; ourSock = null; "
								+ e.getLocalizedMessage() + " | "
								+ e.getStackTrace()[0]);
				disconnect();
				throw e;
	...
	public static String getStatusName(byte b) {
		return b == OFFLINE ? "Offline" : b == HANDSHAKE ? "Handshake"
				: b == ONLINE ? "Online" : b == AWAY ? "Away"
						: b == XA ? "Extended Away" : "Idk.";
	}
	...
	public void disconnect() throws IOException { // should be used with caution
		...
		theirSock = null;
		setStatus(OFFLINE);
		Logger.log(Logger.NOTICE, this, "Disconnect called on " + address
				+ " | Retry in " + (reconnectAt - System.currentTimeMillis()));
	}
	...
	public boolean isFullyConnected() {
		return ourSockOut != null && ourSock != null && ourSock.isConnected()
				&& !ourSock.isClosed() && theirSock != null
